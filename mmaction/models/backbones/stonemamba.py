...
def window_partition(x, window_size):
    ...
    windows = x.permute(0, 2, 4, 3, 5, 1).reshape(-1, window_size[0]*window_size[1], C)
    return windows
...
class MambaVisionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.transformer_block:
            pad_r = (self.window_size[0] - T % self.window_size[0]) % self.window_size[0]
            pad_b = (self.window_size[1] - V % self.window_size[1]) % self.window_size[1]
        ...
        for _, blk in enumerate(self.blocks):
            x = blk(x)
        if self.transformer_block:
            x = window_reverse(x, self.window_size, Tp, Vp)
            if pad_r > 0 or pad_b > 0:
                x = x[:, :, :T, :V].contiguous()
...
class StoneMamba(BaseModule):
    ...
    def forward(self, x):
        ...
        for level in self.levels:
            ...
            # print(x.shape)
        x = self.norm(x)
        x = rearrange(x, '(n m) c t v -> n m c t v', m=M).contiguous()
        return x
...
if __name__ == '__main__':
    ...
    Flops, params = get_model_complexity_info(model, (M, T, V, C), as_strings=False, print_per_layer_stat=False, verbose=False)
